...
public class ItemServiceImpl implements ItemService {
    ...
    public List<ItemWithLastAndNextBookingAndComments> getItems(int userId, int from, int size) throws UserNotFoundException {
        LocalDateTime now = LocalDateTime.now();
        return itemRepo.findAllWithLastAndNextBookingAndComments(userId, now, new ShareitPageRequest(from, size, Sort.by("ItemWithLastAndNextBookingAndComments.id").descending()));
    }
